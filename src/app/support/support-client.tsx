  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok || !data?.ok) {
        ...
        setError(data?.message ?? '문의를 접수하지 못했어요. 잠시 뒤 다시 시도해주세요.');
        return;
      }

      setSent(true);
    } catch {
      setError('문의를 접수하지 못했어요. 잠시 뒤 다시 시도해주세요.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div style={{ width: '390px', height: '844px', margin: '40px auto', position: 'relative', display: 'flex', flexDirection: 'column', background: '#F3F6FB', color: '#1E3557', overflow: 'hidden', borderRadius: '40px', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
        ...
        {sent ? (
          <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px', padding: '0 30px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '12.5px', color: '#8FAAD0', lineHeight: 1.6 }}>관리자가 확인 후 가입하신 아이디로<br/>답변을 드릴게요.</div>
          ...
          <div className="no-scrollbar" style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
            <div style={{ padding: '20px 16px 28px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
              <p style={{ margin: 0, fontSize: '12.5px', color: '#8FAAD0', lineHeight: 1.6 }}>궁금한 점이나 문제가 있으면 남겨주세요.<br/>가입하신 아이디(이메일)로 답변을 보내드려요.</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                <textarea value={content} onChange={(e) => setContent(e.target.value)} maxLength={MAX_INQUIRY_LENGTH} placeholder="문의하실 내용을 자세히 적어주세요" style={{ width: '100%', border: 'none', outline: 'none', borderRadius: '12px', boxShadow: 'inset 0 0 0 1px #E6EDF7', padding: '13px 14px', fontSize: '14px', color: '#1E3557', background: '#fff', resize: 'none', minHeight: '140px', fontFamily: 'inherit', lineHeight: 1.5 }}></textarea>
              </div>
