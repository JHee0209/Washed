  ...
  return (
    <div style={{ width: '100%', maxWidth: '300px', background: '#17233C', borderRadius: '20px', padding: '18px', display: 'flex', flexDirection: 'column', gap: '12px', boxShadow: '0px 20px 44px -12px rgba(8,20,46,.75)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontSize: '14px', fontWeight: 800, color: '#EEF4FD' }}>{t('qr.title')}</span>
      ...
      <div style={{ position: 'relative', aspectRatio: 1, borderRadius: '18px', overflow: 'hidden', background: '#0D1728' }}>
        ...
        {phase === 'denied' && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(224,235,250,.85)', fontSize: '12px', textAlign: 'center', padding: '20px', lineHeight: 1.6, whiteSpace: 'pre-line' }}>
            {t('qr.denied')}
          </div>
        )}
        {phase === 'no-camera' && (
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        ...
        >
          {isPickupPending ? t('common.close') : t('common.cancel')}
        </button>
