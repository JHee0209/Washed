...
import { SettingsLanguagePicker } from '@/components/language-picker';
import {
  disablePush,
  enablePush,
...
export default function SettingsClient({ name, studentId }: SettingsClientProps) {
  ...
  // 언어 선택은 <SettingsLanguagePicker /> 가 들고 있다 — 고른 값은 localStorage
  // 의 washed_lang 에 남으므로(05 P25 · 기기 단위) 이 화면에 상태를 두지 않는다.
  ...
  const turnOnPush = async () => {
    ...
    if (permissionSnapshot() === 'denied') {
      showToast(t('settings.pushAllowInBrowser'));
      return;
    }
    ...
    try {
      ...
      if (subscription) showToast(t('settings.pushTurnedOn'));
      else if (result === 'denied') showToast(t('settings.pushAllowInBrowser'));
      else showToast(t('settings.pushTurnOnFailed'));
    } catch {
      ...
      showToast(t('settings.pushTurnOnFailed'));
    } finally {
  ...
  const turnOffPush = async () => {
    ...
    try {
      ...
      const subscription = await syncPushToggle();
      showToast(subscription ? t('settings.pushTurnOffFailed') : t('settings.pushTurnedOff'));
      setPushPending(false);
      pushBusyRef.current = false;
    }
  };
  ...
  const pickEvidence = (file: File | null) => {
    ...
    if (file.size > MAX_EVIDENCE_BYTES) {
      showToast(t('settings.reportPhotoTooLarge', { limit: MAX_EVIDENCE_LABEL }));
      if (evidenceInputRef.current) evidenceInputRef.current.value = '';
      return;
    }
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      setLaundryMachine(null);
      clearEvidence();
      showToast(t('settings.reportDone'));
    } catch {
      showToast(t('settings.reportNetworkError'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      showToast(t('settings.withdrawFailed'));
    }
  };
    ...
    <>
      ...
      <div className="app-frame" style={{ position: 'relative', display: 'flex', flexDirection: 'column', background: '#F3F6FB', color: '#1E3557', overflow: 'hidden' }}>
        ...
        <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: '8px', padding: '14px 20px 12px', paddingTop: 'max(14px, env(safe-area-inset-top))', background: '#fff', borderBottom: '1px solid #EAF0FA', width: '100%', position: 'relative' }}>
          ...
          <Link href="/notifications" style={{ boxSizing: 'border-box', width: '25px', height: '25px', borderRadius: '8px', position: 'absolute', right: '30px', top: '50%', transform: 'translateY(-50%)', background: `url(${hasUnread ? '/icons/bell-active.svg' : '/icons/bell.svg'}) center / cover no-repeat` }}></Link>
        </div>
        ...
        <div className="no-scrollbar" style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
          <div style={{ padding: '18px 16px 28px', display: 'flex', flexDirection: 'column', gap: '22px' }}>
            ...
            <h1 style={{ margin: 0, fontSize: '25px', fontWeight: 800, letterSpacing: '-0.8px', marginLeft: '5px', marginBottom: '-10px' }}>{t('settings.title')}</h1>
            ...
            {/* 알림 설정 */}
            <div>
              <div className="ghead">{t('settings.groupNotify')}</div>
            ...
            {/* 신고하기 */}
            <div>
              <div className="ghead">{t('settings.groupReport')}</div>
              <div className="group" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <span style={{ fontSize: '12.5px', color: '#8FAAD0' }}>{t('settings.reportIntro')}</span>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {reasonDefs.map((label, idx) => {
                    ...
                    return (
                      <div key={idx} onClick={() => pickReason(label)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '13px 14px', borderRadius: '12px', cursor: 'pointer', background: on ? 'rgba(91,147,224,.10)' : '#fff', boxShadow: `inset 0 0 0 1px ${on ? '#5B93E0' : '#E6EDF7'}` }}>
                        ...
                        <span style={{ fontSize: '14px', fontWeight: 500, color: on ? '#2F63B8' : '#4A5F82' }}>{t(REPORT_REASON_LABEL_KEY[label])}</span>
                      </div>
                    );
                  })}
                  ...
                  {showLaundryFilter && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '12px', borderRadius: '12px', background: '#F7FAFE', boxShadow: 'inset 0 0 0 1px #E6EDF7' }}>
                      ...
                      {showMachineNumbers && (
                        <>
                          <span style={{ fontSize: '11.5px', fontWeight: 700, color: '#5A7CA8', marginTop: '4px' }}>{t('settings.machineNoLabel')}</span>
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '6px' }}>
                            {Array.from({ length: machineCount }, (_, i) => i + 1).map((n) => {
                              const on = laundryMachine === n;
                              return (
                                <div key={n} onClick={() => setLaundryMachine(n)} style={{ textAlign: 'center', padding: '8px 4px', borderRadius: '10px', cursor: 'pointer', fontSize: '12px', fontWeight: 700, background: on ? '#4C86D8' : '#fff', color: on ? '#fff' : '#4A5F82', boxShadow: `inset 0 0 0 1px ${on ? '#4C86D8' : '#E6EDF7'}` }}>{t('settings.machineNo', { no: n })}</div>
                              );
                            })}
                          </div>
                        </>
                  ...
                  {/* 기타 텍스트 입력 */}
                  {reportReason === '기타' && (
                    <textarea placeholder={t('settings.etcPlaceholder')} value={etcText} onChange={(e) => setEtcText(e.target.value)} style={{ border: 'none', outline: 'none', borderRadius: '12px', boxShadow: 'inset 0 0 0 1px #E6EDF7', background: '#F7FAFE', padding: '12px', fontSize: '13.5px', color: '#1E3557', resize: 'none', minHeight: '64px', fontFamily: 'inherit' }}></textarea>
                  )}
                  ...
                  */}
                  {showEvidenceSlot && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <span style={{ fontSize: '11.5px', fontWeight: 700, color: '#5A7CA8' }}>{t('settings.evidenceLabel')}</span>
                      <input
                      ...
                      {evidencePreview ? (
                        <div style={{ position: 'relative', width: '100%', height: '140px', borderRadius: '12px', overflow: 'hidden', border: '1px solid #E6EDF7' }}>
                          ...
                          <img src={evidencePreview} alt={t('settings.evidenceAlt')} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                          <button
                            type="button"
                        ...
                        >
                          <span>{t('settings.evidenceEmpty')}</span>
                          <span style={{ fontSize: '11px' }}>{t('settings.evidenceHint', { limit: MAX_EVIDENCE_LABEL })}</span>
                        </div>
                      )}
                    </div>
                  )}
                ...
                <button onClick={submitReport} disabled={!canSubmit || submitting} style={{ border: 'none', cursor: canSubmit && !submitting ? 'pointer' : 'default', color: canSubmit && !submitting ? '#fff' : '#A8BCD9', background: canSubmit && !submitting ? '#E0554E' : '#EDF2FA', borderRadius: '12px', padding: '13px 16px', fontSize: '14px', fontWeight: 700 }}>{submitting ? t('settings.reportSubmitting') : t('settings.reportSubmit')}</button>
              </div>
            </div>
            ...
            {/* 도움말 */}
            <div>
              <div className="ghead">{t('settings.groupHelp')}</div>
              <div className="group">
                <div className="cell" onClick={() => setFaqOpen(true)} style={{ cursor: 'pointer' }}>
                  <span className="ctitle">{t('settings.faq')}</span>
                  <svg width="8" height="14" viewBox="0 0 8 14" fill="none" style={{ flexShrink: 0 }}><path d="M1.2 1.2 6.6 7l-5.4 5.8" stroke="#B4C2D6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path></svg>
                </div>
                <Link href="/support" className="cell">
                  <span className="ctitle">{t('settings.support')}</span>
                  <svg width="8" height="14" viewBox="0 0 8 14" fill="none" style={{ flexShrink: 0 }}><path d="M1.2 1.2 6.6 7l-5.4 5.8" stroke="#B4C2D6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path></svg>
                </Link>
              </div>
            </div>
            ...
            {/* 버전 정보 */}
            <div>
              <div className="ghead">{t('settings.groupInfo')}</div>
              <div className="group">
                <div className="cell">
                  <span className="ctitle">{t('settings.versionLabel')}</span>
            ...
            <div className="group">
              <button onClick={handleLogout} className="cell" style={{ width: '100%', justifyContent: 'center', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: '14.5px', fontWeight: 600, color: '#2F63B8' }}>{t('common.signOut')}</button>
              <button onClick={() => setWithdrawOpen(true)} className="cell" style={{ width: '100%', justifyContent: 'center', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: '14.5px', fontWeight: 600, color: '#E0554E' }}>{t('settings.withdraw')}</button>
            </div>

            <div style={{ textAlign: 'center', fontSize: '11.5px', color: '#A8BCD9' }}>{t('common.appVersion')}</div>
        ...
        <div style={{ flexShrink: 0, display: 'flex', justifyContent: 'center', padding: '8px 0', background: 'rgba(255,255,255,.92)', backdropFilter: 'blur(16px)', borderTop: '1px solid rgba(112,115,124,.12)', height: '60px' }}>
          <div style={{ display: 'flex', gap: '28px', justifyContent: 'center' }}>
            ...
            <Link href="/settings" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', padding: '5px 16px', borderRadius: '10px', background: 'rgba(0,102,255,.08)' }}>
              ...
              <span style={{ fontSize: '10.5px', fontWeight: 600, color: '#0066FF' }}>{t('nav.settings')}</span>
            </Link>
          </div>
        </div>

        {/* 언어 설정 팝업 모달은 <SettingsLanguagePicker /> 안에 있다 (위 셀과 한 벌) */}
        ...
        {withdrawOpen && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 120, background: 'rgba(20,42,84,.42)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 40px' }}>
            <div style={{ background: '#fff', borderRadius: '16px', padding: '22px 20px 0', width: '100%', display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'center', textAlign: 'center', overflow: 'hidden' }}>
              <span style={{ fontSize: '16px', fontWeight: 700, color: '#1E3557' }}>{t('settings.withdrawTitle')}</span>
              {/* 기간만 굵게 — 문장 가운데 조각이라 {days} 자리에 끼운다 */}
              <span style={{ fontSize: '13px', color: '#8FAAD0', lineHeight: 1.55, paddingBottom: '18px', whiteSpace: 'pre-line' }}>
                <Rich
                  messageKey="settings.withdrawBody"
                  slots={{ days: <b style={{ color: '#2F63B8' }}>{t('settings.withdrawDays')}</b> }}
                />
              </span>
              <div style={{ display: 'flex', width: 'calc(100% + 40px)', borderTop: '1px solid #EDF2F9' }}>
                <button onClick={() => setWithdrawOpen(false)} disabled={withdrawPending} style={{ flex: 1, border: 'none', cursor: withdrawPending ? 'default' : 'pointer', color: '#2F63B8', background: 'transparent', padding: '14px', fontSize: '15px', fontWeight: 600 }}>{t('common.cancel')}</button>
                <div style={{ width: '1px', background: '#EDF2F9' }}></div>
                <button onClick={handleWithdraw} disabled={withdrawPending} style={{ flex: 1, border: 'none', cursor: withdrawPending ? 'default' : 'pointer', color: withdrawPending ? '#A8BCD9' : '#E0554E', background: 'transparent', padding: '14px', fontSize: '15px', fontWeight: 700 }}>{withdrawPending ? t('common.processing') : t('settings.withdrawConfirm')}</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ position: 'absolute', inset: 0, zIndex: 120, background: 'rgba(23,23,23,.45)', display: 'flex', alignItems: 'flex-end' }}>
            <div style={{ background: '#fff', width: '100%', height: '86%', borderRadius: '20px 20px 0 0', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              <div style={{ flex: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px 12px', borderBottom: '1px solid rgba(112,115,124,.12)' }}>
                <span style={{ fontSize: '16px', fontWeight: 800 }}>{t('settings.faqTitle')}</span>
